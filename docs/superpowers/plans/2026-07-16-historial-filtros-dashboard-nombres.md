...
- [ ] **Step 1: Remove the always-hidden table attribute and the separate empty-state div**
...
- [ ] **Step 2: Add the `fechaToYMD` and `fechaDisplay` helpers**
...
- [ ] **Step 3: Add module state and the filter/apply functions; rewrite `renderHistorial()`**
...
- [ ] **Step 1: Add compact CSS for the filter inputs**
...
- [ ] **Step 2: Add the filter row to the table header**
...
- [ ] **Step 3: Add the "Limpiar filtros" button**
...
- [ ] **Step 4: Add `_initFiltrosHistorial` and `limpiarFiltrosHistorial`**
...
- [ ] **Step 5: Hook the init into the load sequence**
...
- [ ] **Step 1: Start a static file server for the Browser pane**
...
- [ ] **Step 2: Verify the Historial filter row (structure + logic)**

- [ ] Navigate to the Historial tab. Confirm a second row of filter controls appears under the headers: text inputs under Folio/Nombre/Calle/Colonia/Placa/Observaciones, dropdowns under Tipo/PGA/Origen, a date picker under Fecha, and empty cells under the checkbox/ID/Hora/Detalle/m³ columns.
- [ ] Confirm the PGA dropdown lists the 5 PGAs (ROVIROSA WADE, LEY SAULO, ESTERITO, RECOVERDE, COMPRESORA) plus "Todos".
- [ ] Inject fake rows and verify filtering via the console (use `javascript_tool`):
  ```javascript
  _historialRows = [
...
- [ ] Check `read_console_messages` for JS errors after these interactions.

- [ ] **Step 3: Verify the Dashboard card (structure)**

- [ ] Navigate to the Dashboard tab. Confirm a "Personas frecuentes (más de 3 entradas en el periodo)" card appears (it will be empty / show its fallback since there's no backend). Inject a render to confirm the template:
  ```javascript
...
- [ ] **Step 5: Final backend suite + syntax check**
...
**Historial:**
- [ ] La tabla muestra una fila de filtros fija debajo de los encabezados
- [ ] Tipo/PGA/Origen usan dropdown con los valores conocidos
- [ ] Fecha usa selector de fecha y coincide con el día exacto (sin desfase de zona horaria)
- [ ] Folio/Nombre/Calle/Colonia/Placa/Observaciones buscan coincidencia parcial, sin distinguir mayúsculas/minúsculas
- [ ] Combinar dos o más filtros aplica todos a la vez (Y lógico)
- [ ] "Seleccionar todos" / "Eliminar seleccionados" solo afectan filas visibles
- [ ] Cambiar un filtro limpia la selección previa
- [ ] "Limpiar filtros" vacía los 10 campos de un clic
- [ ] El pill de conteo refleja el conjunto filtrado, no el total
- [ ] Con 0 coincidencias, el mensaje dice "Ningún registro coincide con los filtros."
- [ ] La fila de filtros sigue visible aunque el filtro no encuentre nada
- [ ] La columna Fecha se muestra como DD/MM/YYYY
- [ ] La exportación a Excel sigue exportando todos los registros

**Dashboard:**
- [ ] Aparece la tarjeta "Personas frecuentes" junto a "Orígenes"
- [ ] Solo cuenta ENTRADA
- [ ] Solo muestra personas con más de 3 entradas en el periodo seleccionado
- [ ] Cambiar el periodo actualiza la tarjeta
- [ ] "Antonio Gómez" y "antonio gomez " cuentan como la misma persona
- [ ] Nombres vacíos nunca aparecen
- [ ] Si nadie supera el umbral, se muestra un mensaje explicativo
